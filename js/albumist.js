		...
		heading.onclick = function(){
			...
			if (!content.innerHTML) {
				...
				loadLyricsAsync(bandName, track, config, function(html) {
					var freebaseHtml = '<img src="' + getImagePath(config) + 'freebase.png">' +
						'<a href="http://freebase.com/view' + track.id + '" target="_blank">' +
							'Visit Song Page on Freebase' +
						'</a><br />';
					var editLyricsHtml = '<img src="' + getImagePath(config) + 'lyricwiki.png"> <a href="' + 
						getEditUrlForLyricWiki(bandName, track, config) + 
						'" target="_blank">' + (html ? 'Edit on LyricWiki' : 'Add to LyricWiki') + '</a><br />';
					if (html) {
						content.setHTML(
							'<div class="no_margin">' + editLyricsHtml + freebaseHtml + '</div><br />' +
							'<div class="no_margin">' + html + '</div><br />'
						);
					} else {
						content.setHTML(
							'<div class="no_margin">' + editLyricsHtml + freebaseHtml + '</div><br />' +
							'<div class="no_margin"><i>' +
								'This song is not on LyricWiki yet, at least not under the URL ' +
								getViewUrlForLyricWiki(bandName, track, config) + '<br />' + 
								'You can <a href="http://lyricwiki.org/Special:Search" target="_blank">Search LyricWiki</a>' +
								'to see if it is there under a different name, and either change the track name' +
								'in Freebase or in LyricWiki to match.  If not, you can add it yourself under this name.' +
							'</i></div><br />'
						);
					}
					collapsible.toggle();
	...
	function openAlbumCore(id, config, cache) {
		...
		var processTracklistResult = function(result) {
			if (result && result.releases) {
				...
				var coverImageEl = new Element("img");
				if (cache[id].musicbrainzId) {
				 	// http://stackoverflow.com/a/9891041/211160
 					coverImageEl.onerror = function() {this.onerror=null;this.src=unavailableLink;}; 
					coverImageEl.setAttribute("src", 'http://coverartarchive.org/release-group/' + cache[id].musicbrainzId + '/front');
				} else {
					coverImageEl.setAttribute("src", unavailableLink);
				}
				coverImageEl.setAttribute("width", config.coverEdgeSize);
				coverImageEl.setAttribute("height", config.coverEdgeSize);
				coverImageEl.setAttribute("border", 0);

				coverImageEl.inject(containerEl);
				var linksEl = new Element("div", {className: "no_margin"});
				var freebaseHtml = (
					'<img src="' + getImagePath(config) + 'freebase.png">' +
					'<a href="http://freebase.com/view' + result.id + '" target="_blank">' +
				...
				for(var i = 0; i < track_list.length; i++) {
					...
					var html = ('[<a href="#"><span>+</span></a>] ' + 
						(i + 1) + ". " + track_list[i].name + " (" + Metaweb.toMinutesAndSeconds(track_list[i].length)+")"
					);
					...
					collapsibles[i] = initializeCollapsibleItem(
						headerEl,
						collapseEl,
						collapseContainerEl,
						result.artist,
				...
				noticeMissingEl.setHTML(
					'<hr><i>Is this information incomplete, or incorrect?<br />' +
					'You can visit Freebase and easily edit tracks and cover art.<br />' +
					'Then reload this page and you will have the option to add lyrics as well!</i>');
...
///////////////////////////////////////////////////////////////////////////////
	...
	function displayCore(element, bandId, bandName, configParam) {
		...
		if (config.injectCss && !cssWasInjected) {
			...
			var cssNode = new Element('link', {type: 'text/css', rel: 'stylesheet', href: getCssPath() + "albumist.css", media: 'screen'});
		...
		function buildAlbumTable(result) {		
			...
			headerColumnEl.setHTML(
				'<div id="hd" style="background-color:#DDDDDD;"><p style="text-align:center;">' +
					'Discography pulled from <a href="' + bandFreebaseURL + '" target="_blank">Freebase</a>' +
					'<img src="' + getImagePath(config) + 'freebase.png' + '" style="vertical-align: middle;" />' +
					'using <a href="http://hostilefork.com/albumist" target="_blank">"Albumist"</a>' +
					'by <a href="http://hostilefork.com">Hostile Fork</a>' +
					'<img src="' + getImagePath(config) + 'hostilefork.png' + '" width="16" height="16" style="vertical-align: middle;" />' +
				'</p></div>'
			);				
			...
			for(var rawAlbumIndex = 0; rawAlbumIndex < albums.length; rawAlbumIndex++) {
				...
				var columnEl = new Element("td", {
					style: "vertical-align:top;",
					width: Math.floor(config.tableWidth/config.albumsPerRow)
				});
				...
				var pictureDiv = new Element("div", {
					className: "album-picture",
					style: "text-align: center; display:block;"
				});
				
				var pictureLink = new Element("a", {
					href: "http://www.freebase.com/view" + album.id, target: "_blank"
				});

				var thumbImageEl = new Element("img");
				...
				if (musicbrainzId === null) {
					thumbImageEl.setAttribute("src", unavailableLink);
 					cache[album.id].musicbrainzId = null;
				} else {
 					cache[album.id].musicbrainzId = musicbrainzId;
 					// http://stackoverflow.com/a/9891041/211160
 					thumbImageEl.onerror = function() {this.onerror=null;this.src=unavailableLink;};
				   	thumbImageEl.setAttribute("src",
				   		'http://coverartarchive.org/release-group/' + musicbrainzId + '/front-250'
				   	);
				}
				thumbImageEl.setAttribute("width", config.thumbEdgeSize);
				thumbImageEl.setAttribute("height", config.thumbEdgeSize);
				thumbImageEl.setAttribute("border", 0);
				...
				var	functionString = (
					'return' + ' ' + ('document.getElementById("' + element + '").openAlbum') + 
					'(' + '"' + album.id + '"' + ');'
				);
				
				// Intercept links if we can.  If not, they will default to Freebase
				setOnClickFunctionString(thumbImageEl, functionString);
				
				thumbImageEl.inject(pictureLink);
				pictureLink.inject(pictureDiv);
				
				pictureDiv.inject(albumDiv);
				
				var titleDiv = new Element("div", {
					className: "album-title", style: "text-align: right; font-size: 10px;"
				});
			...
			footerColumnEl.setHTML(
				'<div id="hd" style="background-color:#DDDDDD;"><p style="text-align:center;">' +
					'<a href="http://hostilefork.com/albumist" target="_blank">"Albumist"</a>' +
...
///////////////////////////////////////////////////////////////////////////////
	...
	Albumist.Config = function() {
		...
		this.injectCss = true;
		this.basePath = "";
		this.imagePath = null; /* null means default to basePath + 'media/'; */
